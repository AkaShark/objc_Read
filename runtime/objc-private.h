...
#   define ISA_MAGIC_VALUE 0x000001a000000001ULL
    struct {
        uintptr_t nonpointer        : 1; // 是否开启isa优化（TaggedPointer）
        uintptr_t has_assoc         : 1; // 是否存在关联应用
        uintptr_t has_cxx_dtor      : 1; // 表示对象是否有C++或者Objc的析构器
        uintptr_t shiftcls          : 33; // MACH_VM_MAX_ADDRESS 0x1000000000 类的指针 arm64架构中有33位可以储存类指针
        uintptr_t magic             : 6; // 判断对象是否初始化完成，在arm64中0X16是调试器判断当前对象是真的对象还是没有初始化的空间
        uintptr_t weakly_referenced : 1; // 对象被指向或者曾经指向一个ARC的弱变量 没有弱引用的对象可以更快的释放
        uintptr_t deallocating      : 1; // 对象是否正在释放内存
        uintptr_t has_sidetable_rc  : 1; // 判断对象的引用计数是否过大，如果过大则需要其他散列表来进行存储
        uintptr_t extra_rc          : 19; // 存储该对象的引用计数值减一后的结果 对象的引用计数超过1 会存储在这个里面 如果引用计数为10 该值为9.
